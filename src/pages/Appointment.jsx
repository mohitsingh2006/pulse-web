...
import Reschedule from "../components/appointment/RescheduleModal";
...
import OldAppointment from "../components/appointment/OldAppointment";
...
const Appointment = () => {
    ...
    const AppointmentDetailModalRef = useRef(null);
    const AppointmentDetailModalInstance = useRef(null);



    const [showRescheduleModal, setShowRescheduleModal] = useState(false);
    const rescheduleModalRef = useRef(null);
    const rescheduleModalInstance = useRef(null);

    const fetchAppointment = async () => {
        try {
            const response = await axiosInstance.get(
                `${API_URL}appointments/get_appointments`
            );
    ...
    useEffect(() => {
        ...
        if (AppointmentDetailModalInstance.current) {
            if (showAppointmentDetailModal) {
                AppointmentDetailModalInstance.current.show();
            } else {
                AppointmentDetailModalInstance.current.hide();
            }
        }

         if (rescheduleModalInstance.current) {
            if (showRescheduleModal) {
                rescheduleModalInstance.current.show();
            } else {
                rescheduleModalInstance.current.hide();
            }
        }
    }, [
    ...
    useEffect(() => {
        ...
        if (appointmentModalRef.current) {
            appointmentModalInstance.current = new Modal(
                appointmentModalRef.current,
                {}
            );
        }

        if (AppointmentDetailModalRef.current) {
            AppointmentDetailModalInstance.current = new Modal(
                AppointmentDetailModalRef.current,
                {}
            );
        }

         if (rescheduleModalRef.current) {
            rescheduleModalInstance.current = new Modal(
            ...
            <main className="px-3 pb-5 appointment-page">
                <div className="row ">
                    <div className="col-12">
                        <div className="bg-white nav-pills-outer mb-4 border-bottom d-flex justify-content-between py-2 ps-2">
                            ...
                            <div className="d-flex align-items-center gap-3 pe-3 ">
                                ...
                                <Link
                                    ...
                                    className="btn dr-btn  py-2 shadow-none"
                                >
                                    <svg
                                ...
                                <Link
                                    className="btn py-2 shadow-none queue position-relative"
                                    to="/appointment/patientqueue"
                                >
                ...
                <div
                    className="modal fade create-appointment-modal"
                    ref={AppointmentDetailModalRef}
                    onHide={() => setShowAppointmentDetailModal(false)}
                    data-bs-backdrop="static"
                    dataBsKeyboard="false"
                    tabIndex="-1"
                >
                    <div className="modal-dialog modal-xl">
                        <AppointmentDetailModal
                            handleCloseModal={() => setShowAppointmentDetailModal(false)}
                        />
                    </div>
                </div>
                ...
                >
                    <div className="modal-dialog modal-xl">
                        <RescheduleModal
                            handleCloseModal={() => setShowRescheduleModal(false)}
                        />
                    </div>
                </div>
            </main>
